...
const mainListenersMap = new Map();
// Set up a single listener for all main process events
ipcRenderer.on(IPC_EVENT_CHANNEL_NAME, (_event, channel, ...args) => {
...
const createMainEventHandler = (scope, eventName) => {
...
// };
// --- Process main events ---
const mainEvents = Object.fromEntries(Object.entries(eventsMeta.main).map(([scope, eventNames]) => [
    scope,
    Object.fromEntries(eventNames.map(eventName => {
