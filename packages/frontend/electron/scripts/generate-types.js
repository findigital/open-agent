...
import { generateApiTypesFile, generateCombinedMetaFile, generateEventTypesFile, } from './ipc-generator/utils';
...
const paths = {
    // Generate api types under @afk/electron-api
    ...
    Object.values(paths)
        ...
        try {
            ...
            if (e.code !== 'EEXIST') {
                ...
                throw new Error(`Failed to create directory for ${filePath}: ${e.message}`);
            }
        }
    });
}
function writeGeneratedFiles(collectedApiHandlers, collectedEvents, paths) {
    ...
    fs.writeFileSync(paths.ipcMeta, generateCombinedMetaFile(collectedApiHandlers, collectedEvents));
...
function generateIpcDefinitions() {
    ...
    if (collectedApiHandlers.size === 0) {
        console.log('No IPC handlers found. Generated files will be empty or contain minimal structure.');
    }
