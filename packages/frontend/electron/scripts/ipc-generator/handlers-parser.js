 ...
 */
function parseIpcHandleDecorator(memberDeclaration) {
            ...
            if (scopeInitializer) {
                if (Node.isStringLiteral(scopeInitializer)) {
                    ...
                    if (propertyAccessType.isStringLiteral()) {
                        ...
                        const constant = checker.compilerObject.getConstantValue(scopeInitializer.compilerNode);
 ...
 */
export function parseIpcHandlers(project) {
    const collectedApiHandlers = new Map();
    project.getSourceFiles().forEach(sourceFile => {
        sourceFile.getClasses().forEach(classDeclaration => {
            ...
            members.forEach(memberDeclaration => {
                ...
                const className = classDecl?.getName?.() || 'UnknownClass';
                ...
                if (srcIdx !== -1) {
                    modulePath =
                        '@afk/electron/' + absPath.substring(srcIdx + '/src/'.length);
                }
                ...
                if (handlerScopeMethods) {
                    ...
                    console.error(`[CRITICAL] Failed to retrieve handler methods array for scope: ${scope}`);
                }
            });
        });
    });
...
//# sourceMappingURL=handlers-parser.js.map