 ...
 */
export function parseIpcEventDecorator(propertyDeclaration) {
    const decorator = propertyDeclaration
        .getDecorators()
        .find(d => d.getName() === IpcEventDecoratorName);
    if (!decorator)
        return { error: 'Decorator not found' };
        ...
        if (initializer) {
            if (Node.isStringLiteral(initializer))
                ...
                if (type.isStringLiteral())
                    scopeValue = type.getLiteralValue();
                else
                    scopeValue = initializer.getNameNode().getText();
            }
        }
    }
    if (!scopeValue)
    ...
    if (propertyTypeNode) {
        ...
        const knownStreamTypesRegex = /(?:BehaviorSubject|ReplaySubject|Subject|Observable|EventEmitter)<([^>]+)>/;
    ...
    if (payloadType === 'any[]') {
        ...
        const typesToInspect = [propertyType];
        ...
        for (const typeToInspect of typesToInspect) {
            ...
            if (!typeToInspect.isClassOrInterface() &&
 ...
 */
export function parseIpcEvents(project) {
    ...
    project.getSourceFiles().forEach(sourceFile => {
        sourceFile.getClasses().forEach(classDeclaration => {
            classDeclaration.getProperties().forEach(propertyDeclaration => {
                ...
                const { scope, eventName, payloadType, entry, filePath, originalPropertyName, } = parsedEventInfo;
                ...
                if (idx !== -1) {
                    modulePath =
                        '@afk/electron/' + absPath.substring(idx + '/src/'.length);
                }
                modulePath = modulePath.replace(/\.[tj]sx?$/, '');
                const clsName = propertyDeclaration.getParent()?.getName?.() || 'UnknownClass';
                ...
                const description = propertyDeclaration
                    ...
                    .filter(Boolean)
                    .join('\n');
                if (!collectedEvents.has(scope))
                    collectedEvents.set(scope, []);
                const eventScopeMethods = collectedEvents.get(scope);
                if (eventScopeMethods) {
                    const existingEvent = eventScopeMethods.find(event => event.eventName === eventName);
                    if (existingEvent) {
                        throw new Error(`[Event ERR] Duplicate event found for scope '${scope}' and eventName '${eventName}'.\n` +
                            `  Original: ${existingEvent.filePath} (${existingEvent.className}.${existingEvent.propertyName})\n` +
                            `  Duplicate: ${filePath} (${clsName}.${propName})`);
                    }
                    ...
                    console.error(`[CRITICAL] Failed to retrieve event methods array for scope: ${scope}`);
                }
            });
        });
    });
...
//# sourceMappingURL=events-parser.js.map