export const determineEntry = (path) => {
    if (path.includes('src/entries/helper'))
        return 'helper';
    return 'main';
};
/**
 ...
 */
export function generateApiTypesFile(collectedApis) {
 ...
 */
export function generateEventTypesFile(collectedEvents) {
    ...
    for (const [scope, events] of collectedEvents) {
        ...
        for (const evt of events) {
            const capName = evt.eventName.charAt(0).toUpperCase() + evt.eventName.slice(1);
 ...
 */
export function generateCombinedMetaFile(apiHandlers, events) {
    let content = `// AUTO-GENERATED FILE. DO NOT EDIT MANUALLY.\n// Generated by: packages/frontend/apps/electron/scripts/generate-types.ts\n/* eslint-disable @typescript-eslint/no-explicit-any */\n`;
    ...
    apiHandlers.forEach((methods, scope) => {
        methods.forEach(m => {
            ...
            if (!handlersMeta[ent])
                handlersMeta[ent] = {};
            if (!handlersMeta[ent][scope])
                handlersMeta[ent][scope] = [];
    ...
    events.forEach((evtList, scope) => {
        evtList.forEach(e => {
            ...
            if (!eventsMeta[ent])
                eventsMeta[ent] = {};
            if (!eventsMeta[ent][scope])
                eventsMeta[ent][scope] = [];
            eventsMeta[ent][scope].push(e.eventName);
        });
    });
    // Serialize handlersMeta & eventsMeta
    content += `export const handlersMeta = ${JSON.stringify(handlersMeta, null, 2)} as const;\n\n`;
